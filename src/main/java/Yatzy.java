...
public class Yatzy {
    ...
    private static int sumTargetValues(@NotNull IntStream dices, int targetValue) {
    ...
    public static int pair(int d1, int d2, int d3, int d4, int d5) {
        ...
        int toto = a.entrySet().stream().filter(entry -> entry.getValue() >= 2).map(Map.Entry::getKey).max(Integer::compareTo).orElse(0);
